...
<div class="container">
    ...
    <div class="action-buttons">
        ...
        <a href="mood?action=viewTrends" class="action-btn back"><i class="fas fa-arrow-left"></i> Back to Calendar</a>
    </div>
</div>
...
<script>
    function editMood() {
        ...
        window.location.href = 'mood?action=edit&id=<%= dailyMood.getId() %>';
    }
    ...
    function deleteMood() {
        // Make API call to delete mood
        fetch('mood?action=delete&id=<%= dailyMood.getId() %>', {
            method: 'POST'
        }).then(response => {
            if (response.ok) {
                alert('Mood entry deleted successfully!');
                window.location.href = 'mood?action=viewTrends';
            } else {

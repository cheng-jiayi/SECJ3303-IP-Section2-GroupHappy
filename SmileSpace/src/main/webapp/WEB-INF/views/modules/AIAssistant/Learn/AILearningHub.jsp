...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .module-header.beginner {
            background: linear-gradient(135deg, #4CAF50, #45a049);
        }
        .module-header.intermediate {
                        ...
                        <c:choose>
                            <c:when test="${not empty param.category && param.category != 'all'}">
                                ...
                                <span style="font-size: 14px; color: #8B7355; margin-left: 10px;">
                                    Showing ${fn:length(modules)} modules
                                    <c:if test="${not empty stats.categoryStats[param.category]}">
                                        (${stats.categoryStats[param.category]} total in this category)
                                    </c:if>
                                </span>
                            </c:when>
                    ...
                    <c:forEach var="module" items="${modules}">
                        <div class="module-card">
                            <div class="module-header 
                                <c:choose>
                                    <c:when test="${module.level == 'Beginner'}">beginner</c:when>
                                    <c:when test="${module.level == 'Intermediate'}">intermediate</c:when>
                                    <c:otherwise>advanced</c:otherwise>
                                </c:choose>">
                                <span class="module-category">${module.category}</span>
                                <span class="module-level">${module.level}</span>
                                <i class="fas 
                                    <c:choose>
                                        <c:when test="${module.level == 'Beginner'}">fa-seedling</c:when>
                                        <c:when test="${module.level == 'Intermediate'}">fa-chart-line</c:when>
                                        <c:otherwise>fa-mountain</c:otherwise>
                                    </c:choose> module-icon"></i>
                            </div>
                            <div class="module-body">
                                <h3>${module.title}</h3>
                                <p class="module-description">
                                    <c:choose>
                                        <c:when test="${not empty module.description}">${module.description}</c:when>
                                ...
                                <div class="module-meta">
                                    <span><i class="fas fa-clock"></i> 
                                        <c:choose>
                                            <c:when test="${not empty module.estimatedDuration}">${module.estimatedDuration}</c:when>

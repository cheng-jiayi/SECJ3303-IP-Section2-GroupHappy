...
public class FeedbackDAO {
    ...
    private final RowMapper<Feedback> feedbackRowMapper = (rs, rowNum) -> {
        ...
        try {
            feedback.setHasUnseenReply(rs.getBoolean("has_unseen_reply"));
            feedback.setUserReply(rs.getString("user_reply"));
            
            Timestamp userReplyTimestamp = rs.getTimestamp("user_reply_date");
            if (userReplyTimestamp != null) {
                feedback.setUserReplyDate(new Date(userReplyTimestamp.getTime()));
            }
            
            Timestamp lastUpdatedTimestamp = rs.getTimestamp("last_updated");
            if (lastUpdatedTimestamp != null) {
                feedback.setLastUpdated(new Date(lastUpdatedTimestamp.getTime()));
            }
        } catch (SQLException e) {
            System.err.println("New columns not found (expected on first run): " + e.getMessage());
        }
    ...
    public boolean updateFeedbackReply(int feedbackId, String replyMessage, Integer replierUserId) {
        String sql = "UPDATE feedback SET reply_message = ?, reply_date = CURRENT_TIMESTAMP, " +
                    "is_resolved = TRUE, has_unseen_reply = TRUE, last_updated = CURRENT_TIMESTAMP " +
                    "WHERE feedback_id = ?";
    ...
    public boolean addUserReply(int feedbackId, String replyMessage, int userId) {
        String sql = "UPDATE feedback SET " +
                    "user_reply = ?, " +
                    "user_reply_date = CURRENT_TIMESTAMP, " +
                    "is_resolved = FALSE, " +  
        ...
        try {
            int affectedRows = jdbcTemplate.update(sql, 
                replyMessage, feedbackId, userId);
            
            if (affectedRows > 0) {
                logHistory(feedbackId, userId, "USER_REPLY", 
                    "User replied: " + (replyMessage.length() > 50 ? 
                    replyMessage.substring(0, 50) + "..." : replyMessage));
                return true;
            }
        } catch (Exception e) {
            System.err.println("Error adding user reply: " + e.getMessage());
            e.printStackTrace();
        }
        return false;
    }
    ...
    public int getUnseenReplyCount(int userId) {
        String sql = "SELECT COUNT(*) FROM feedback WHERE user_id = ? AND has_unseen_reply = TRUE";
        
        try {
            Integer count = jdbcTemplate.queryForObject(sql, Integer.class, userId);
            return count != null ? count : 0;
        } catch (Exception e) {
            System.err.println("Error getting unseen reply count: " + e.getMessage());
            return 0;
        }
    }
    
    public boolean markRepliesAsSeen(int feedbackId, int userId) {
        String sql = "UPDATE feedback SET has_unseen_reply = FALSE WHERE feedback_id = ? AND user_id = ?";
        
        try {
            int affectedRows = jdbcTemplate.update(sql, feedbackId, userId);
            return affectedRows > 0;
        } catch (Exception e) {
            System.err.println("Error marking replies as seen: " + e.getMessage());
            return false;
        }
    }
...
public List<Feedback> getUserFeedbackWithFilters(int userId, String searchTerm, String status) {
    StringBuilder sql = new StringBuilder(
        "SELECT f.*, u.full_name as user_full_name, u.user_role " +
        "FROM feedback f " +
        "LEFT JOIN users u ON f.user_id = u.user_id " +
    ...
    if (searchTerm != null && !searchTerm.trim().isEmpty()) {
        sql.append(" AND (f.message LIKE ? OR f.category LIKE ? OR f.reply_message LIKE ? OR f.user_reply LIKE ?)");
        String searchParam = "%" + searchTerm + "%";
        params.add(searchParam);
        params.add(searchParam);
        params.add(searchParam);
    ...
    try {
        ...
        System.err.println("Error getting filtered user feedback: " + e.getMessage());
        e.printStackTrace();
        return new ArrayList<>();
    }
}
